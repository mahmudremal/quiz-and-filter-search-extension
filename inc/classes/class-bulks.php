...
class Bulks {
	...
	protected function setup_hooks() {
		$this->args = [
			...
			'column'	=> 'status',
			'options'	=> [
				''       => __( 'All', 'quiz-and-filter-search-domain' ),
				'open'   => __( 'Open', 'quiz-and-filter-search-domain' ),
				'closed' => __( 'Closed', 'quiz-and-filter-search-domain' ),
			]
		];
		/**
		 ...
		 */
		add_action( 'wp_ajax_futurewordpress/project/quizandfiltersearch/status/switch', [ $this, 'ajax_switch' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/quizandfiltersearch/status/switch', [ $this, 'ajax_switch' ], 10, 0 );
	}
	/**
	...
	public function manage_posts_columns( $columns ) {
		...
		$columns[ $this->args[ 'column' ] ] = __( 'Status', 'quiz-and-filter-search-domain' );
		return $columns;
	}
	/**
	...
	public function manage_posts_custom_column( $column, $post_id ) {
		if( $this->args[ 'column' ] == $column ) {
			$args = [
				'confirm'	=> true,
				'request'	=> [
					'action'		=> 'futurewordpress/project/quizandfiltersearch/status/switch',
					'post_id'		=> $post_id,
					'status'		=> false
				]
			];
