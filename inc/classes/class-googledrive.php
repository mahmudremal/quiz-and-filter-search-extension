...
class GoogleDrive {
	...
	protected function setup_hooks() {
		...
		add_action( 'wp_ajax_futurewordpress/project/quizandfiltersearch/action/submitarchives', [ $this, 'submitArchives' ], 10, 0 );
		add_action( 'wp_ajax_futurewordpress/project/quizandfiltersearch/action/deletearchives', [ $this, 'deleteArchives' ], 10, 0 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/filesystem/ziparchives', [ $this, 'zipArchives' ], 10, 2 );
		add_action( 'futurewordpress/project/quizandfiltersearch/googledrive/fetchauth', [ $this, 'fetchAndSaveAuthCode' ], 0, 1 );

		add_filter( 'futurewordpress/project/quizandfiltersearch/socialauth/link', [ $this, 'goForAuthCode' ], 10, 2 );
	}
	public function initialize() {
		$this->redirectURL					= apply_filters( 'futurewordpress/project/quizandfiltersearch/socialauth/redirect', '/handle/google', 'google' );
		$this->parentDirectory			= apply_filters( 'futurewordpress/project/quizandfiltersearch/system/getoption', 'auth-googledrivefolder', '1MliOhH16m413OmiBGJM90cTmWUcwNUP4' );
		$this->clientSecret					= apply_filters( 'futurewordpress/project/quizandfiltersearch/system/getoption', 'auth-googleclientsecret', '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' );
		$this->clientId							= apply_filters( 'futurewordpress/project/quizandfiltersearch/system/getoption', 'auth-googleclientid', '719552948296-d14739tknbd33tv16b932pb859kcvums.apps.googleusercontent.com' );
	}
	public function developmentMode() {
		...
		try{
			...
			// $drive_file_move = $this->uploadFileWithMeta( QUIZ_AND_FILTER_SEARCH_BUILD_PATH . '/img/img-to-contract.jpg' );
	...
	public function submitArchives() {
		...
		$archive_path = apply_filters( 'futurewordpress/project/quizandfiltersearch/filesystem/uploaddir', false ) . '/' . $fileName . '.zip';
		...
		if( $result ) {
			...
			wp_send_json_success( [ 'message' => __( 'Archived Successfully.', 'quiz-and-filter-search-domain' ), 'hooks' => [ 'reload-page' ] ], 200 );
		} else {
			wp_send_json_error( __( 'Problem detected while creating archive.', 'quiz-and-filter-search-domain' ), 200 );
		}
	}
	public function deleteArchives() {
		global $wpdb;if( ! apply_filters( 'futurewordpress/project/quizandfiltersearch/system/isactive', 'general-archivedelete' ) ) {return;}
		...
		wp_send_json_success( [ 'message' => __( 'Archive removed from server successfully!', 'quiz-and-filter-search-domain' ), 'hooks' => [ 'reload-page' ] ], 200 );
	}
    ...
    if ($http_code == 204) {
			...
			$error_msg = ( $http_code == 404 ) ? __( 'File Not found', 'quiz-and-filter-search-domain' ) : __( 'Failed to delete file from drive.', 'quiz-and-filter-search-domain' );
			throw new \Exception('Error '.$http_code.': ' . $error_msg );
    }
	}
	public function createDirectory( $folderName ) {
		...
		$response = json_decode( $response, true );
		// print_r( $response );
		$error_msg = __( 'Error creating folder:', 'quiz-and-filter-search-domain' ) . ( ! empty( curl_error( $curl ) ) ? ': ' . curl_error( $curl ) : '' );
		curl_close($curl);
		if (curl_errno($curl)) {
