 ...
 */
 class Helpers {
	...
	protected function setup_hooks() {
		if( ! defined( 'QUIZ_AND_FILTER_SEARCH_OPTIONS' ) ) {define( 'QUIZ_AND_FILTER_SEARCH_OPTIONS', (array) get_option( 'quiz-and-filter-search-domain', [] ) );}
		add_filter( 'futurewordpress/project/quizandfiltersearch/system/getoption', [ $this, 'getOption' ], 10, 2 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/system/isactive', [ $this, 'isActive' ], 10, 1 );

		add_filter( 'futurewordpress/project/quizandfiltersearch/database/countries', [ $this, 'databaseCountries' ], 10, 2 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/database/countryflags', [ $this, 'countryFlags' ], 10, 1 );

		add_filter( 'futurewordpress/project/quizandfiltersearch/user/dashboardpermalink', [ $this, 'dashboardPermalink' ], 10, 2 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/user/visitorip', [ $this, 'visitorIP' ], 10, 0 );


		add_filter( 'futurewordpress/project/quizandfiltersearch/filter/server/time', [ $this, 'serverTime' ], 10, 2 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/filter/string/random', [ $this, 'generateRandomString' ], 10, 2 );

		add_filter( 'futurewordpress/project/quizandfiltersearch/filesystem/filemtime', [ $this, 'filemtime' ], 10, 2 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/filesystem/uploaddir', [ $this, 'uploadDir' ], 10, 2 );
		add_filter( 'futurewordpress/project/quizandfiltersearch/mailsystem/sendmail', [ $this, 'sendMail' ], 10, 1 );

		add_filter( 'futurewordpress/project/quizandfiltersearch/notices/manager', [ $this, 'noticeManager' ], 10, 3 );
		
		add_action( 'wp_ajax_futurewordpress/project/quizandfiltersearch/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/quizandfiltersearch/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_futurewordpress/project/quizandfiltersearch/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/quizandfiltersearch/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'admin_post_futurewordpress/project/quizandfiltersearch/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
		add_action( 'admin_post_nopriv_futurewordpress/project/quizandfiltersearch/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
    ...
    return isset( QUIZ_AND_FILTER_SEARCH_OPTIONS[ $option ] ) ? QUIZ_AND_FILTER_SEARCH_OPTIONS[ $option ] : $default;
	}
	/**
    ...
    return ( isset( QUIZ_AND_FILTER_SEARCH_OPTIONS[ $option ] ) && QUIZ_AND_FILTER_SEARCH_OPTIONS[ $option ] == 'on' );
	}
	/**
	...
	public function uploadDir( $file = false, $force = false ) {
		...
		if( $this->theUploadDir === false ) {
			...
			$uploadDir[ 'basedir' ] = $uploadDir[ 'basedir' ] . '/' . apply_filters( 'futurewordpress/project/quizandfiltersearch/system/getoption', 'ftp-mediadir', 'futurewordpress' );
			$uploadDir[ 'baseurl' ] = $uploadDir[ 'baseurl' ] . '/' . apply_filters( 'futurewordpress/project/quizandfiltersearch/system/getoption', 'ftp-mediadir', 'futurewordpress' );
	...
	public function uploadFile() {
		check_ajax_referer( 'futurewordpress/project/quizandfiltersearch/verify/nonce', '_nonce' );
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'quiz-and-filter-search-domain' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'quiz-and-filter-search-domain' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				// $meta['type'] = apply_filters( 'futurewordpress/project/quizandfiltersearch/validate/format', $meta['type'], $meta );
				$oldMeta[] = $meta;
				WC()->session->set( 'uploaded_files_to_archive', $oldMeta );
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'quiz-and-filter-search-domain' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'quiz-and-filter-search-domain' );
				wp_send_json_error( $error, 200 );
			}
		}
		wp_send_json_error( __( 'Error happens.', 'quiz-and-filter-search-domain' ) );
	}
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'quiz-and-filter-search-domain' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'quiz-and-filter-search-domain' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'quiz-and-filter-search-domain' ), __( '404 not found', 'quiz-and-filter-search-domain' ) );
		}
	}
	...
	public function dashboardPermalink( $id, $user = 'me' ) {
		...
		$profile = ( apply_filters( 'futurewordpress/project/quizandfiltersearch/system/getoption', 'permalink-userby', 'id' ) == 'id' ) ? FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK . '/' . ( ( $id ) ? $id : 'me' ) : FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK . '/' . $user;
		return $profile . '/' . apply_filters( 'futurewordpress/project/quizandfiltersearch/profile/defaulttab', 'profile' );
	}
