...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		$this->file = QUIZ_AND_FILTER_SEARCH__FILE__;
		$this->plugin_slug = 'quiz-and-filter-search-domain';
		$this->plugin_name = 'quiz-and-filter-search-domain';
		$this->textdomain = 'quiz-and-filter-search-domain'; // str_replace('_', '-', $plugin_slug);
	...
	public function init() {
		$this->general = (object) wp_parse_args( apply_filters( 'futurewordpress/project/quizandfiltersearch/settings/general', [] ), [
			'page_title'					=> __( 'Configuration.', 'quiz-and-filter-search-domain' ),
			'menu_title'					=> __( 'Config', 'quiz-and-filter-search-domain' ),
			'role'								=> 'manage_options',
			'slug'								=> $this->plugin_slug,
			'page_header'					=> __( 'customization page.', 'quiz-and-filter-search-domain' ),
			'page_subheader'			=> __( 'Your setting panel from where you can control and customize.', 'quiz-and-filter-search-domain' ),
			'no_password'					=> __( 'A password is required.', 'quiz-and-filter-search-domain' ),
		] );
	...
	public function add_settings_link( $links ) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __( 'Settings', 'quiz-and-filter-search-domain' ) . '</a>';
		// array_push( $links, $settings_link );
		$links[] = $settings_link;
		// $links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __( 'Developer Support', 'quiz-and-filter-search-domain' ) . '</a>';
		return $links;
	}
	...
	private function settings_fields() {
		$settings = [];
		$settings = apply_filters( 'futurewordpress/project/quizandfiltersearch/settings/fields', $settings );
		return $settings;
	}
	...
	public function display_field( $args ) {
		...
		switch( $field['type'] ) {
			...
			case 'radio':
			case 'select_multi':
				$html .= apply_filters( 'futurewordpress/project/quizandfiltersearch/settings/fields/label', '<br/><span class="description">' . $field['description'] . '</span>', $field );
			break;

			default:
				$html .= apply_filters( 'futurewordpress/project/quizandfiltersearch/settings/fields/label', '<label for="' . esc_attr( $field['id'] ) . '"><span class="description">' . $field['description'] . '</span></label>' . "\n", $field );
			break;
		}
