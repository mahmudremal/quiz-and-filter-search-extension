		...
		init_events() {
			...
			document.body.addEventListener('gotcategorypopupresult', async (event) => {
				...
				if(thisClass.Swal.isVisible()) {
					...
					setTimeout(() => {
						if(thisClass.lastJson.category!=''&&thisClass.lastJson.category.length>=1) {
			...
			document.body.addEventListener('category_updated', (event) => {
		...
		init_popup() {
			...
			Swal.fire({
				...
				didOpen: async () => {
					...
					formdata.append('action', 'futurewordpress/project/ajax/edit/category');
